...
AsmFunctionsWrapper.prototype.initAsm = function(heapSize) {
    ...
    // becouse wanted not to allocate a new heap on every call,
    // so it reuse a static variable
    ...
    // specifically do this here so that it only recreate
    // the asm functions if it really have to
...
function asmFunctions(stdlib, foreign, heapBuffer) {
    ...
    function lowPass(lastOutput, currentInput, smoothingFactor) {
        ...
        // +x represents double
        // all the arithmetic ared done, using doubles rather than floats,
    ...
    function resonate(heapStart, heapEnd) {
        ...
        // by making the smoothing factor large, the cutoff frequency
        // is made very low, acting as just an offset remover
        ...
        for (i = heapStart << 2; (i|0) <= (heapEnd << 2); i = (i + 4)|0) {
            ...
            // Not sure why, but the resonating process plays (study area under development)
            // havok with the DC offset - it jumps around everywhere.
            // then puting it back to zero DC offset by adding a high-pass
    ...
    function fadeTails(heapStart, length) {
        ...
        tailProportion = 0.1;
        // let's first convert length from an int to an unsigned (>>>0)
        // so that, convert it a double for the argument of floor()
        ...
        // the result of an additive addition is an int-ish,
    ...
    function renderKarplusStrong(
        ...
        // where will be taking samples from the last period, after
        ...
        */
        ...
        for (targetIndex = 0;
            ...
            if ((targetIndex|0) < (periodSamples|0)) {
                ...
                // by varying 'pluck damping', the spectral content 
                // of the input noise could be controled
                curInputSample =
                ...
                // if stringTension == 1.0, it would be reading from the
                // same sample it was writing to
                // ordinarily, this would have the effect that only the first
                // period of noise was preserved, and the rest of the buffer
                // would be silence, but because it was reusing the heap,
                // it has actually be reading samples from old waves
                curInputSample = 0.0;
            }
